...
import type { EventReport, EventReportForm } from '@/types'
...
export const useEventReportStore = defineStore('eventReport', () => {
  ...
  const fetchEvents = async (branch: string) => {
    ...
    error.value = null
    try {
      const url = `${getApiUrl('eventReportsList')}?branch=${encodeURIComponent(branch)}`
  ...
  const submitEventReport = async (form: EventReportForm) => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('eventReportSubmit'), {
        method: 'POST',
