    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full z-50 animate-scale-in relative max-h-[90vh] overflow-y-auto">
        ...
        <div v-if="showSuccess" class="text-center py-12 px-6">
          ...
          <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ $t('event.submitted', 'Event Report Submitted!') }}</h2>
          <p class="text-gray-600 mb-8">{{ $t('event.submitSuccess', 'Your event report has been successfully submitted and saved to our system.') }}</p>
          <div class="flex gap-4 justify-center">
            ...
            >
              {{ $t('event.submitAnother', 'Submit Another') }}
            </button>
          </div>
        </div>
        ...
        <div v-else>
          ...
          <div class="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 rounded-t-lg">
            <h2 class="text-2xl font-bold">{{ $t('event.submitReport', 'Submit Event Report') }}</h2>
            <p class="text-white/90 text-sm">{{ $t('event.shareDetails', 'Share details about events happening in your branch') }}</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ $t('event.branchName', 'Branch Name') }} *
              </label>
              <input
                type="text"
                :value="form.branch"
              ...
              />
              <p class="text-xs text-gray-500 mt-1">{{ $t('event.autofilled', 'Auto-filled from your session') }}</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                :placeholder="$t('event.titlePlaceholder', 'e.g., Meditation Workshop')"
              />
            ...
            <div>
              ...
              <textarea
                ...
                :placeholder="$t('event.descriptionPlaceholder', 'Describe what happened at the event...')"
              ></textarea>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ $t('event.participants', 'Participant Details') }} *
              </label>
              <input
                ...
                :placeholder="$t('event.participantsPlaceholder', 'e.g., 50 participants from different organizations')"
              />
            ...
            <button
              ...
              class="w-full py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity flex items-center justify-center gap-2"
            >
              ...
              {{ isSubmitting ? $t('event.submitting', 'Submitting...') : $t('event.submit', 'Submit Report') }}
            </button>
          </form>
        </div>
      </div>
...
// Initialize branch from store
watch(
  () => props.isOpen,
  (newVal) => {
    if (newVal) {
      form.branch = branchStore.selectedBranch
      resetForm()
    }
  }
)
